 ...
 * @package trydo
 */
if( !class_exists('trydo_Recent_Post') ){
    class trydo_Recent_Post extends WP_Widget{
        /**
        ...
        $title = ( ! empty( $instance['title'] ) ) ? $instance['title'] : esc_html__( 'Recent Posts','trydo' );
                                    ...
                                    <ul class="blog-meta">
                                        ...
                                        <?php if ($show_read_time):
                                            if (function_exists('trydo_content_estimated_reading_time')){ ?>
                                                <li><?php echo trydo_content_estimated_reading_time(get_the_content()); ?></li>
                                            <?php  } ?>
                                        <?php endif ?>
                                    </ul>
                                <?php }  ?>
    ...
    ?>
        <p><label for="<?php echo esc_attr($this->get_field_id( 'title' )); ?>"><?php echo esc_html__( 'Title:','trydo' ); ?></label>
        <input class="widefat" id="<?php echo esc_attr($this->get_field_id( 'title' )); ?>" name="<?php echo esc_attr($this->get_field_name( 'title' )); ?>" type="text" value="<?php echo esc_attr($title); ?>" /></p>
        
        
        <p>
            <label for="<?php echo esc_attr($this->get_field_id( 'show_item' )); ?>"><?php echo esc_html__( 'No. of Item of posts to show:','trydo' ); ?></label>
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id( 'num_title_word' )); ?>"><?php echo esc_html__( 'Title Word','trydo' ); ?></label>
        ...
        <p>
            ...
            <label for="<?php echo esc_attr($this->get_field_id( 'show_date' )); ?>"><?php echo esc_html__( 'Display post date?','trydo' ); ?></label>
        </p>
        <p>
            ...
            <label for="<?php echo esc_attr($this->get_field_id( 'show_read_time' )); ?>"><?php echo esc_html__( 'Display Read Time?','trydo' ); ?></label>
        </p>
    <?php
        }
    }
...
function trydo_Recent_Post(){
    register_widget('trydo_Recent_Post');
}
add_action('widgets_init','trydo_Recent_Post');
