...
if (!function_exists('rbt_sharing_icon_links')) {
    function rbt_sharing_icon_links()
    {
        ...
        $html .= '<li><a href="' . esc_url($facebook_url) . '" target="_blank" class="aw-facebook"><i class="fab fa-facebook-f"></i> ' . esc_html__("Facebook", "trydo") . '</a></li>';

        // twitter
        $twitter_url = 'https://twitter.com/share?' . esc_url(get_permalink()) . '&amp;text=' . get_the_title();
        $html .= '<li><a href="' . esc_url($twitter_url) . '" target="_blank" class="aw-twitter"><i class="fab fa-twitter"></i> ' . esc_html__("Twitter", "trydo") . '</a></li>';

        // linkedin
        $linkedin_url = 'http://www.linkedin.com/shareArticle?url=' . esc_url(get_permalink()) . '&amp;title=' . get_the_title();
        $html .= '<li><a href="' . esc_url($linkedin_url) . '" target="_blank" class="aw-linkdin"><i class="fab fa-linkedin-in"></i> ' . esc_html__("Linkedin", "trydo") . '</a></li>';
...
if (!function_exists('rbt_portfolio_sharing_icon_links')) {
    function rbt_portfolio_sharing_icon_links()
    {
        ...
        if ($trydo_options['trydo_enable_portfolio_share'] == 'yes') {
            echo wp_kses_post($html);
        }
